 ...
 * </ul>
 *
 * <p>Subclasses must implement {@link #results()} and {@link #setDriverMode(boolean)} to handle
 * camera-specific operations. Measurement uncertainty is automatically adjusted based on:
 *
 * <ul>
...
public abstract class CameraIO {
   ...
   *
   * <p>Initializes the {@link PhotonPoseEstimator} with the AprilTag field layout and multi-tag PnP
   * strategy for robust pose estimation.
   *
  ...
  public CameraIO(VisionConstants.CameraConfig config) {
    this.config = config;
    this.estimator = new PhotonPoseEstimator(VisionConstants.AprilTag.kLayout, config.bot2cam);
  }

  /**
   * Gets the latest pipeline results from the camera.
   *
   * <p>Implemented by subclasses to handle camera-specific result retrieval (e.g., USB cameras vs
   * coprocessor cameras).
   *
