...
public class SuperstructureIOSpark implements SuperstructureIO {
    ...
    public SuperstructureIOSpark() {
        ...
        feederConfig.encoder
            .positionConversionFactor((2.0 * Math.PI) / feederMotorReduction) // Rotor Rotations -> Roller Radians
            .velocityConversionFactor(
                (2.0 * Math.PI) / 60.0 / feederMotorReduction
            )
        ...
        intakeLauncherConfig.encoder
            .positionConversionFactor(
                (2.0 * Math.PI) / intakeLauncherMotorReduction
            ) // Rotor Rotations -> Roller Radians
            .velocityConversionFactor(
                (2.0 * Math.PI) / 60.0 / intakeLauncherMotorReduction
            )
            .uvwMeasurementPeriod(10)
            .uvwAverageDepth(2);
        tryUntilOk(intakeLauncher, 5, () ->
    ...
    public void updateInputs(SuperstructureIOInputs inputs) {
        ifOk(feeder, feederEncoder::getPosition, value ->
            inputs.feederPositionRad = value
        );
        ifOk(feeder, feederEncoder::getVelocity, value ->
            inputs.feederVelocityRadPerSec = value
        );
        ifOk(
            feeder,
            new DoubleSupplier[] {
                feeder::getAppliedOutput,
                feeder::getBusVoltage,
            },
            values -> inputs.feederAppliedVolts = values[0] * values[1]
        );
        ifOk(feeder, feeder::getOutputCurrent, value ->
            inputs.feederCurrentAmps = value
        );

        ifOk(intakeLauncher, intakeLauncherEncoder::getPosition, value ->
            inputs.intakeLauncherPositionRad = value
        );
        ifOk(intakeLauncher, intakeLauncherEncoder::getVelocity, value ->
            inputs.intakeLauncherVelocityRadPerSec = value
        );
        ifOk(
            intakeLauncher,
            new DoubleSupplier[] {
        ...
        ifOk(intakeLauncher, intakeLauncher::getOutputCurrent, value ->
            inputs.intakeLauncherCurrentAmps = value
        );
    }
