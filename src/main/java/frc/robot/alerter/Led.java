...
public class Led {
     ...
     *
     * <br>Each priority has an associated numeric level, with higher numbers indicating higher priority.
     *
     ...
     */
    public synchronized void register(BooleanSupplier active, Mode mode, Priority priority) {
    ...
    private void rainbow() {
        // use HSL
        double increment = 360.0 / LED_COUNT;
        double offset = (Timer.getFPGATimestamp() * RAINBOW_SPEED) % 360;

        for (int i = 0; i < LED_COUNT; i++) {
            double hue = (i * increment + offset) % 360;
